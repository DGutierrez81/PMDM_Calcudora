...
class MainActivity : AppCompatActivity() {
    ...
    private fun initListeners(){
        ...
        borrar.setOnClickListener{ borrado() }
    }
    ...
    private fun muestraValor(pantalla : String, detalle : String){
        if(bandera){
            ...
            txtDetalle.text = detalle.substring(0, detalle.length -1)
        }
    ...
    private fun borrado(){
        ...
        /*
        when {
            calc.numTemp1.isNotEmpty() && calc.numTemp2.isEmpty()-> {
                calc.numTemp1 = calc.numTemp1.substring(0, calc.numTemp1.length - 1)
                muestraValor(calc.numTemp1, calc.numTemp1)
            }

            calc.numTemp2.isNotEmpty() -> {
                calc.numTemp2 = calc.numTemp2.substring(0, calc.numTemp2.length - 1)
                muestraValor(calc.numTemp2, calc.numTemp2)
            }

            calc.numTemp1.isEmpty() || calc.numTemp2.isEmpty() -> {
                mensajeError("No existe nada que borrar.")
            }
        }
